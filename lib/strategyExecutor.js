...
module.exports.prototype.authenticate= function(strategies, scope, trace, request, response, callback) {
  ...
  if( !this.strategies || this.strategies.length ==0 ) {
    trace( "Unable to find a strategy to authenticate with", request, scope, "###");
    ...
    try{
      ...
      trace( "Error choosing strategy: "+ e, request, scope, "###");
      callback(e);
    }
    if( carryOn ) {
      if( strategiesToTest.length  == 0 ) {
        trace( "Tested all strategies :" + util.inspect(executionScope.executionResult), request, scope, "###");
        callback( null, executionScope.executionResult );
      }
      else {
       ...
       ;(function next(e) {
          if (executionScope.executionResult.halted || e || complete === total) {
            trace( "Tested all strategies", request, scope, "###");
            ...
            if( strategy.isValid === undefined || strategy.isValid() ) {  
              trace( "Attempting authentication with: " + strategy.name, request, scope, "###" );
