...
exports.OAuthServices.prototype.accessToken= function(request, protocol, callback) {
  ...
  if(!validateParameters(requestParameters, ['oauth_consumer_key',  
...
exports.OAuthServices.prototype.authenticate = function(request, protocol, callback){  var parsedUrl= url.parse(request.url, true);
  var method= request.method;
  var headers= request.headers;  var host= headers['host'];
  var path= parsedUrl.pathname;
  var query= parsedUrl.query;
  var self = this;
  var requestParameters= parseParameters(method, headers, query, request.body);
...
exports.OAuthServices.prototype.authorize= function(request, protocol, callback) {
  ...
  var self = this; 
  // if the token is an empty string then it is a 2-legged oauth
  if(requestParameters['oauth_token']===''){
    var self = this;
    // Fetch the secret and token for the user
    self.provider.applicationByConsumerKey(requestParameters['oauth_consumer_key'], function(err, user) {
      if(err) {
        callback(new errors.OAuthProviderError('Invalid Consumer Key'), null);
      } else {
        if(user.consumer_key == null || user.secret == null) {
	      callback(new errors.OAuthProviderError("provider: applicationByConsumerKey must return a object with fields [consumer_key, secret]"));
	      return;
        }
        ...
        self.provider.cleanRequestTokens(requestParameters['oauth_consumer_key'], function(err, result) {
          // If we have a user for this consumer key let's calculate the signature
          var oauthClient= new oauth.OAuth(null, null, user.consumer_key, user.secret, null, null, 'HMAC-SHA1');
          parsedUrl.protocol= protocol+":";
          parsedUrl.host= host;
          var reconstructedUrl= url.format(parsedUrl);
          var reconstructedOauthParameters= oauthClient._normaliseRequestParams(requestParameters);
          var calculatedSignature= oauthClient._getSignature(method, reconstructedUrl, reconstructedOauthParameters, '' );
