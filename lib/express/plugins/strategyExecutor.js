     ...
     if( !this.strategies || this.strategies.length ==0 ) callback( null, executionResult );
       ...
       try{
         for(var i =0 ;i < strategies.length; i++ ) {
           strategiesToTest[strategiesToTest.length]= this.strategies.get(strategies[i]);
         }
         carryOn= true;
       }
       catch(e) {
         ...
         if( strategiesToTest.length  == 0 ) callback( null, executionResult );
          ...
          ;(function next(e) {
             if (executionResult.halted || e || complete === total)
               ...
               strategy.executionResult= executionResult; // Horribly flawed as we're sharing strategy instances atm.
               if( strategy.isValid() ) {
