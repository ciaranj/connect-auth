...
exports.DigestStrategy= AuthStrategy.extend({
   ...
   authenticate: function(request, callback) {
     ...
     self._getPasswordForUser(username, function(error, password){
       if(error) callback(error);
         ...
         if( myResponse == response ) {
           self.success(username, callback);
         }
         else {
           if( self.embedded ) self.fail(callback);
           else self._unAuthenticated(request, callback)
         }
       }
     })
